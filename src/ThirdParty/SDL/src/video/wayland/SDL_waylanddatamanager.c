...
ssize_t 
Wayland_data_source_send(SDL_WaylandDataSource *src,
                         const char *mime_type, int fd)
{
    ...
    mime_data = mime_data_list_find(&src->mimes,
                                                      mime_type);
...
int Wayland_data_source_add_data(SDL_WaylandDataSource *src,
                                 const char *mime_type,
                                 const void *buffer,
                                 size_t length) 
{
    ...
    if (length > 0) {
        ...
        if (internal_buffer == NULL) {
            ...
            status = mime_data_list_add(&src->mimes, mime_type,
...
SDL_bool 
Wayland_data_source_has_mime(SDL_WaylandDataSource *src,
                             const char *mime_type)
{
    SDL_bool found = SDL_FALSE;

    if (src != NULL) {
        found = mime_data_list_find(&src->mimes, mime_type) != NULL;
    }
    return found;
}

void* 
Wayland_data_source_get_data(SDL_WaylandDataSource *src,
                             ...
                             SDL_bool null_terminate)
{
    ...
    *length = 0;
    ...
    if (src == NULL) {
        SDL_SetError("Invalid data src");
    } else {
        mime_data = mime_data_list_find(&src->mimes, mime_type);
...
void
Wayland_data_source_destroy(SDL_WaylandDataSource *src)
{
    if (src != NULL) {
        wl_data_source_destroy(src->src);
        mime_data_list_free(&src->mimes);
        SDL_free(src);
    }
}
                                  ...
                                  SDL_WaylandDataSource *src)
{
    ...
    if (data_device == NULL) {
        ...
        status = SDL_SetError("Invalid src");
    } else {
        SDL_MimeDataList *mime_data = NULL;

        wl_list_for_each(mime_data, &(src->mimes), link) {
            wl_data_source_offer(src->src,
                                 mime_data->mime_type); 
        ...
        if (num_offers == 0) {
            ...
            data_device->selection_source = src;
        }
    }
