...
/**
 *  Retrieves the last SDL error and, if existent, prints
 *  it using \c SDL_Log()
 * */
...
void RunGame()
{
    ...
    while (loop)
    {
        ...
        if (millis >= 1000)
        {
            ...
            double msPerFrame = (double)millis/frames;
            title += " " + std::to_string(msPerFrame) + "ms/frame  ";
...
void CheckSDLError(int line = -1)
{
    ...
    SDL_LogCritical(SDL_LOG_CATEGORY_APPLICATION, errorStr.c_str());
}
