...
Wayland_SetClipboardText(_THIS, const char *text)
{
    ...
    if (_this == NULL || _this->driverdata == NULL) {
        ...
        if (text[0] != '\0') {
            SDL_WaylandDataSource* src = Wayland_data_source_create(_this);
            Wayland_data_source_add_data(src, TEXT_MIME, text,
                                         strlen(text) + 1); 

            status = Wayland_data_device_set_selection(data_device, src);
            if (status != 0) {
                Wayland_data_source_destroy(src);
            }
        } else {
